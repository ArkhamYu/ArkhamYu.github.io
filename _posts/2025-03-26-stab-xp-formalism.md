...
<figure>
  <img src="/assets/img/qec/stabformalism.png"
       alt="Repetition code circuit used to illustrate the stabilizer formalism and measurement constraints."
       style="max-width: 100%;">
  <figcaption><strong>Figure 1.</strong> Repetition code circuit used to illustrate the stabilizer formalism and measurement constraints.</figcaption>
</figure>
...
$$
\mathcal{S}_0
= \langle Z_1,\; Z_5,\; Z_2 Z_3,\; Z_3 Z_4,\; X_2 X_3 X_4 \rangle .
$$

Recall the conjugation action of a CNOT gate \(\mathrm{CX}_{c \to t}\) on single-qubit Pauli operators (ignoring global phases):
$$
\begin{aligned}
X_c &\mapsto X_c X_t, &\qquad Z_c &\mapsto Z_c, \\
X_t &\mapsto X_t,     &\qquad Z_t &\mapsto Z_c Z_t .
\end{aligned}
$$
...
Z_1         &\mapsto Z_1 Z_2, \\
Z_5         &\mapsto Z_5, \\
Z_2 Z_3     &\mapsto Z_2 Z_3, \\
Z_3 Z_4     &\mapsto Z_3 Z_4, \\
X_2 X_3 X_4 &\mapsto (X_2 X_1) X_3 X_4 = X_1 X_2 X_3 X_4 .
\end{aligned}
$$
Thus
$$
\mathcal{S}_1
= \langle Z_1 Z_2,\; Z_5,\; Z_2 Z_3,\; Z_3 Z_4,\; X_1 X_2 X_3 X_4 \rangle .
$$
...
$$
\begin{aligned}
Z_1 Z_2         &\mapsto (Z_3 Z_1) Z_2 = Z_1 Z_2 Z_3, \\
Z_5             &\mapsto Z_5, \\
Z_2 Z_3         &\mapsto Z_2 Z_3, \\
Z_3 Z_4         &\mapsto Z_3 Z_4, \\
X_1 X_2 X_3 X_4 &\mapsto X_1 \cdot (X_3 X_1) \cdot X_2 X_4 = X_2 X_3 X_4 .
\end{aligned}
$$
Thus
$$
\mathcal{S}_2
= \langle Z_1 Z_2 Z_3,\; Z_5,\; Z_2 Z_3,\; Z_3 Z_4,\; X_2 X_3 X_4 \rangle .
$$
...
Z_2 Z_3         &\mapsto Z_2 Z_3, \\
Z_3 Z_4         &\mapsto Z_3 Z_4, \\
X_2 X_3 X_4     &\mapsto X_2 \cdot (X_3 X_5) \cdot X_4 = X_2 X_3 X_4 X_5 .
\end{aligned}
$$
Thus
$$
\mathcal{S}_3
= \langle Z_1 Z_2 Z_3,\; Z_3 Z_5,\; Z_2 Z_3,\; Z_3 Z_4,\; X_2 X_3 X_4 X_5 \rangle .
$$
...
\begin{aligned}
Z_1 Z_2 Z_3         &\mapsto Z_1 Z_2 Z_3, \\
Z_3 Z_5             &\mapsto Z_3 (Z_4 Z_5) = Z_3 Z_4 Z_5, \\
Z_2 Z_3             &\mapsto Z_2 Z_3, \\
Z_3 Z_4             &\mapsto Z_3 Z_4, \\
X_2 X_3 X_4 X_5     &\mapsto X_2 X_3 (X_4 X_5) X_5 = X_2 X_3 X_4 .
\end{aligned}
$$
...
- \(Z_1 = (Z_1 Z_2 Z_3)(Z_2 Z_3) \in \mathcal{S}_{\mathrm{final}}\)  
  \(\Rightarrow\) \(M_1 = Z_1\) is deterministic with outcome \(+1\).
- \(Z_5 = (Z_3 Z_4 Z_5)(Z_3 Z_4) \in \mathcal{S}_{\mathrm{final}}\)  
  \(\Rightarrow\) \(M_2 = Z_5\) is deterministic with outcome \(+1\).
- \(Z_2 Z_3 = +1\) and \(Z_3 Z_4 = +1\) enforce
  \(m_3 \oplus m_4 = 0\) and \(m_4 \oplus m_5 = 0\), i.e. \(m_3 = m_4 = m_5\).  
  The individual values are random, but subject to these parity constraints.

These parity checks correspond to the two detectors \((m_3 \oplus m_4)\) and  
\((m_4 \oplus m_5)\) in the detector error model.
...
and
$$
P := \mathrm{diag}(1,\omega^2),
\qquad
P^N = I.
$$

Each choice of precision \(N\) leads to a different stabiliser formalism.  
For example, \(N = 2\) corresponds to the standard Pauli stabiliser formalism,  
with \(\omega = i\) and \(Z^2 = I\).
...
Let
\[
\mathbf{u} = (p \mid \mathbf{X} \mid \mathbf{Z})
...
$$
XP_N(\mathbf{u})
  := \omega^p \bigotimes_{0 \leq i < n} X^{\mathbf{X}[i]} P^{\mathbf{Z}[i]} .
$$
...
The XP formalism has a fundamental commutation relation that allows us to move
\(P\) operators to the right of \(X\) operators:
$$
PX = \omega^2 X P^{-1}.
$$
...
we can **rescale** \(A\) and write it as a precision-\(2\) operator  
