...
$$
\lvert 0\rangle_{1} \otimes \bigl(a\lvert 000\rangle + b\lvert 111\rangle\bigr)_{2,3,4} \otimes \lvert 0\rangle_{5},
$$
...
- \(Z_2 Z_3 = +1\) and \(Z_3 Z_4 = +1\) enforce
  \(m_3 \oplus m_4 = 0\) and \(m_4 \oplus m_5 = 0\),  
  i.e. \(m_3 = m_4 = m_5\).  
...
$$
\omega := \exp\!\Bigl(\frac{\pi i}{N}\Bigr)
       = \exp\!\Bigl(\frac{1}{2N} 2\pi i\Bigr),
\qquad \text{so that } \omega^{2N} = 1,
$$
and
$$
...
Let \(\mathbf{u} = (p \mid \mathbf{X} \mid \mathbf{Z}) \in
\mathbb{Z}_{2N} \times \mathbb{Z}_2^n \times \mathbb{Z}_N^n\)  
be an integer vector of length \(2n + 1\).  
The XP operator of precision \(N\) corresponding to \(\mathbf{u}\) is
$$
XP_N(\mathbf{u}) := \omega^p \bigotimes_{0 \leq i < n}
...
XP_N\bigl(p \bmod 2N \,\big|\, \mathbf{X} \bmod 2 \,\big|\, \mathbf{Z} \bmod N\bigr) .
$$
...
The **action of an XP operator** on a computational-basis state \(\lvert \mathbf{e}\rangle\),  
where \(\mathbf{e} \in \mathbb{Z}_2^n\), is
$$
XP_N(p \mid \mathbf{X} \mid \mathbf{Z}) \lvert \mathbf{e}\rangle
...
Then \(\omega = \exp\!\bigl(\tfrac{1}{16} 2\pi i\bigr)\) and \(P^8 = I\),  
so \(P = T\) where \(T = \mathrm{diag}(1,\sqrt{i})\).  
The overall phase of the operator is
\(\omega^{12} = \exp\!\bigl(\tfrac{12}{16} 2\pi i\bigr) = -i\).
...
&= \exp\!\Bigl(\frac{12}{16} 2\pi i\Bigr)\, X_1 X_2 X_3 T_3^4 \\
&= -i\, X_1 X_2 X_3 Z_3 .
\end{aligned}
$$
